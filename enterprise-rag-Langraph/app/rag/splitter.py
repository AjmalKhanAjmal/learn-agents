...
class RecursiveTextSplitter(BaseTextSplitter):
    def __init__(
        self, chunk_size=settings.CHUNK_SIZE, chunk_overlap=settings.CHUNK_OVERLAP
    ):
        self.splitterrrr = RecursiveCharacterTextSplitter(
            chunk_size=chunk_size, chunk_overlap=chunk_overlap
        )

    def split(self, cleaned_text: str) -> list[str]:
        try:
            documents = self.splitterrrr.create_documents([cleaned_text])
